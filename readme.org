...
to reproduce the numerical experiments described in [[https://arxiv.org/abs/2405.17259]["The state learner -- a
super learner for right-censored data" (Munch & Gerds)]]. Details are given in
[[./experiments/readme-experiments.org]]. The article describes a real data example,
but as we cannot share the data from that study, the tutorial below uses an
emulated data set with the same structure as the original data analysed in the
article.
