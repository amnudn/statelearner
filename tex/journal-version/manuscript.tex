...
%% Packages
\usepackage{amsthm, dsfont, amssymb,soul,xcolor,
  % amsfonts,
  enumitem,
  amsmath,graphicx,verbatim,pgfplots,tikz,prodint,booktabs}
...
An approach which avoids a pre-specified censoring model was proposed
...
theoretical guarantees seem to exist for this procedure, and it has
not yet been extended to the situation with competing risks.

The result of the state learner algorithm could be a medical risk
...
\citep{kattan2000pretreatment}. We introduce our notation and
framework in Section~\ref{sec:framework}. In
Section~\ref{sec:super-learning} we define general super learning for
right-censored data. Section~\ref{sec:super-learner-simple} introduces
the state learner, and Section~\ref{sec:theor-results-prop} provides
theoretical guarantees. In Section~\ref{sec:targeted-learning} we
...
and~\ref{sec:state-learner-with} contain proofs.  An implementation of
the state learner is available at
\url{https://github.com/amnudn/statelearner} along with codes for
reproducing our numerical experiments.
...
data according to a distribution motivated from a real data set in
...
five learners: a Nelson-Aalen, three Cox regression models
...
\begin{figure}
  ...
  \caption[]{The results of applying the 125 combinations of learners to the
    ...
    learner', `Mortality learner', and `Censoring learner', respectively.}
