...
%% TODO: Update figures with name 'figure-'.
%% Write macro for including figure or not.
...
\begin{abstract}
  ...
  % based on a model for the censoring distribution.
  % Unfortunately, the
  ...
  % pre-specified estimator of the censoring distribution. To relax this,
...
Medical decision making is traditionally based on the discrepancy between
intuitive judgment and the law of chance \citep{redelmeier1995probability}. We
are concerned with the statistical models that provide probabilistic predictions
about the medical future of a patient. We have two different applications of
these models in mind.
% THOMAS: I think we only really have one in mind in this article. For
% stand-alone prediction, I think some further things should be
% addressed/discussed, which I had thought could be for another article. But
% maybe we can but it in here.
One is as a stand-alone tool where the model provides
information for a freshly diagnosed patient. The other is as input to an
algorithm which estimates a causal target parameter such as an average treatment
effect. The models are learned on data from previous patients. A super learner
is a machine learning algorithm that combines a finite set of learners into a
meta learner by estimating prediction performance in hold-out samples using a
pre-specified loss function \citep{van2007super}. When the aim is to make a
prediction model, super learners combine strong learners, such as Cox regression
models and random survival forests \citep{gerds2021medical}.
...
\citep{graf1999assessment, hothorn2006survival, gerds2006consistent}. A disadvantage of inverse probability of
censoring weighted loss estimators is that they require a
pre-specified model for the censoring
distribution. In \cite{westling2021inference} this problem is tackled by
iterating between super learning of the censoring distribution and the
event time
distribution. 
% ANDERS: mangler afrundning efter sidste saetning. eller maaske kan saetningen
% flyttes laenger ned?
% 
% THOMAS: Tror du har skrevet den her bid. Har forsøgt at afrunde.
It is not clear, however, whether such a procedure converges towards an element
...
learner is based on the observable
variables. % ANDERS: hvad mener vi med observable quantities? distribution of
% the observed random variables?
% 
% THOMAS: Changed to 'variables'. It is important that the loss is based on the
% observed variables directly, not just their distribution.
...
\citep{van2007super}.
% ANDERS: er der bedre referencer fra Mark paper/bog for discrete super learner?
% 
% THOMAS: Yes, har taget Hubbard+laan super learner article i stedet.
In the presence of competing risks, our algorithm uses separate
libraries to model the cumulative hazard functions of the competing risks and the
censoring distribution. We show that the oracle selector of the state
learner is consistent if all libraries contain a consistent learner
and prove a finite sample oracle inequality. The result of the state
learner algorithm is a model which predicts the probability of an
event based on covariates in the presence of competing risks
\citep{gerds2021medical}.
% THOMAS: This is not the output directly, but can easily be obtained.
The other application is in targeted
...
continuous for all \( x \) and \( j \). 
% ANDERS: need a note here which says that this is not a limitation?
% 
% THOMAS: I think it is already there in 'For ease of notation...', but I have
% added the next sentence. We could also just work with the product integral,
% would that be better?
This is not a limitation as everything caries over directly to the general case
and is solely imposed to simplify the formulas. We denote by \(S\) the
conditional event-free survival function:
\begin{equation}
  \label{eq:surv-def}
  S(t \mid x)=\exp\left\{-\Lambda_{1}(t \mid x)-\Lambda_{2}(t \mid x)\right\}.
\end{equation}
% ANDERS: I prefer to have the \tau explicit
% 
% THOMAS: OK
...
% ANDERS: can we not allow Gamma to be discrete?
% 
% THOMAS: Yes, no problem, it is only for notation convenience. 
For notational convenience, we assume that \( \Gamma(\blank \mid x) \) is
continuous for all \( x \), and let
\(G(t \mid x)=\exp\left\{-\Gamma(t \mid x)\right\}\) denote the
survival function of the conditional censoring distribution. The distribution 
is characterized by
\begin{equation}\label{eq:parametrizeP}
  ...
  \end{split}
\end{equation}
% ANDERS: H is missing in the model?
% 
% THOMAS: H is part of Q (see equation between equation 1 and 2).
Hence, we may write
...
It seems however that this loss function is not perfectly suited for
super learning, because it cannot be applied to important survival
learners: The Kaplan-Meier estimator, the random survival forest, and
the semi-parametric Cox regression model provide cumulative hazard
functions that are piece-wise constant in the time argument, and hence
assign zero probability to event times not observed in the training
data. This implies that when event times are observed in continuous
time, any of these learners will almost surely have infinite loss in
an independent test set according to the negative partial
log-likelihood loss.
% ANDERS: would be good to have a formula for a the negative partial
% log-likelihood loss showing that the result is infitity
%
% THOMAS: I would argue that this should be clear from the previous sentence and
% the definition of the log-likelihood function: If a discrete model assigns a
% probability of zero to an event, then the log-likelihood of that event is
% log(0) = -infinity.
%
% Such a formula was part of one of the earlier versions where I had put more
% focus on the problem than on the solution. If we add this, we should work with
% the product integral notation.

When a proportional hazards model is assumed, the baseline hazard
function can be profiled out of the likelihood to give a new partial
log-likelihood loss \citep{cox1972regression}, which has been
suggested as a loss function for super learning
\citep{golmakani2020super,verweij1993cross}. While this allows the
library of learners to include Cox' proportional hazard models, the
drawback is that the library is now restricted to include only these
models.
% ANDERS: can the library really include multiple proportional hazards models?
% only one of them can be correct ... also, would be good to merge this
% paragraph with the previous somehow
%
% THOMAS: Yes, it can include multiple PH models (if I understand you
% correctly). A major point is that there is no proof that this works, and when
% you start to think about it, it is not completely obvious whether it should
% work or not. I think it works and am trying to prove it.
%
% I disagree about merging this section with the previous one, I see them as
% describing two completely different approaches and issues.
...
observed
data.
% ANDERS: would be good to add one sentence about how to obtain a prediction
% model from such a ranking.
%
% THOMAS: I think this depends on whether or not we want prediction to be one of
% the tasks addressed in this article. I have focused on something else. We
% could add something like:
Risk predictions for causes one and two can then be obtained from
\( \Lambda_1 \) and $\Lambda_2$ \citep{benichou1990estimates}. To formally
...
\end{figure}
% ANDERS: Figuren er lavet med prodlim, ik? du kan angive nummer for boksene (0
% = initial, 1 = cause 1, 2= cause 2 og jeg ville foretraekke 3 i stedet for -1
% for censored)
%
% THOMAS: Jeg foretrækker -1 frem for 3, så vil gerne høre dine argumenter for
% 3.
...
% ANDERS: the next sentence requires conditional independent censoring and
% restriction to [0,\tau], right?
%
% THOMAS: Yes, added now.
...
determines \( (F, H) \),
% ANDERS: and we should refer to equations (2) and (6)
%
% THOMAS: Yes, added above -- OK?
...
\( \mathcal{H} = \{F_P : P \in \mathcal{P}\} \) where \( F_P \) is defined as in
...
\begin{proposition}
  ...
  \begin{equation*}
    F^* = \argmin_{F \in \mathcal{H}} P_0{[\bar{B}_\tau(F, \blank)]}.
  ...
  \end{equation*}
  % ANDERS: would be good to define the set over which F is minimized with a
  % symbol rather than only words
  %
  % THOMAS: How about now? \mathcal{H} is now defined just before this proposition.
...
Corollary~\ref{cor:oracle-prop} is in essence a special case of Theorem 2.3 in
\cite{vaart2006oracle}.
% ANDERS: is Theo 2.3 correct?
%
% THOMAS: Yes. What do you mean?
We assume that we split the data into equally sized folds, and for simplicity of
presentation we take \( n \) to be such that \( |\data_n^{-k}| = n/K \) with
\( K \) fixed. We will allow the number of learners to grow with \( n \) and
write
\( \mathcal{F}_n=\mathcal{F}(\mathcal{A}_{1,n}, \mathcal{A}_{2,n},
\mathcal{B}_n)\) as short-hand notation and to emphasise the dependence on
\( n \).
...
% \( B \).
In the following we let the space \( \mathcal{H} \) be equipped with the norm
\( \| \blank \|_{P_0} \) defined as
% ANDERS: norm on which space?
%
% THOMAS: Fixed.
\begin{equation}
  \label{eq:norm}
...
% ANDERS: would be good to separate 2-3 subsection in this section. here is a
% vague proposal. above could be under \subsection{Oracle property of the state
% learner}
%
% THOMAS: OK, I have added a suggestion for two sections more. 


% \subsection{Transience of double-robustness}
\subsection{Transience of second order remainder structure}
% THOMAS: It is not clear that double-robustness is preserved, but a second
% order product structure is, so I have changed the title. 
% 
% In Section~\ref{sec:targeted-learning} we demonstrate how the state learner can
% be used for targeted learning.
A targeted estimator is typically obtained from
...
using equation~(\ref{eq:7}). The key feature of a targeted estimator
% ANDERS: at this point we have not defined "targeted estimator" does fisher
% 2021 define this? Bickel does not ...
%
% THOMAS: I would think that Fisher+Kennedy does define it. But my solution to
% this problem was to reference Section 7, which you have removed.
% Alternatively, this could be part of Section 7, but I am quite sure you once
% moved it from there to here. What do you suggest?
is that it is asymptotically equivalent to a sum of i.i.d.\ random variables
plus a second order remainder term
\citep{bickel1993efficient,fisher2021visually}. For the setting with competing
...
% ANDERS: need a sentence a la: Before we discuss the role of the
% state learner for targeted learning we review some results from
% semiparametric efficiency theory. The reader may want to skip this ...
%
% THOMAS: Hmm, I think this is already stated below ('Using tools from
% semi-parametric efficiency theory...'). But we could add something like the
% following? Depending on the framing of the paper (with focus exclusively on
% state learner for targeted learning or also on prediction?), I don't think the
...
% ANDERS: is sub-optimal = too slow?
%
% THOMAS: Hmm, we could use 'too slow', but it is then unclear why it is too
% slow -- too slow for what? Writing 'sub-optimal' suggests (too me at least),
% that there exists a rate that is better -- i.e., this rate it not the optimal
% one for this problem, but an optimal one exists.
...
\(\{O_i\}_{i=1}^n\).  Under a Donsker class condition
% ANDERS: Donsker class condition on what set?
%
% THOMAS: Things are quite informal here so I don't see the point in making this
% precise.
we have the
...
% ANDERS: why is this abuse of notation?
%
% THOMAS: Because we should write \Lambda_j(t \mid (a, w)), because \Lambda as
% originally defined only takes two arguments and not three.
%
% We abuse notation slightly by writing
% \( \Lambda_j(t \mid x) = \Lambda_j(t \mid a, w) \) and
% \( S(t \mid x) = S(t \mid a, w) \) when \( x=(a, w) \).
We use $\mu$
...
fully observed for all individuals. We use an super learner to
...
\end{equation*}
Under a set of
% ANDERS: better to give these assumptions a name: positivity, ... and refer to
% Helene's paper?
%
% THOMAS: Alternatively, we could just call it a 'standardized risk difference'
% and then not discuss counterfactuals, but say that it can be given a causal
% interpretation and reference some papers? I would like to not discuss
% assumptions from causality as it is not very important for the state learner.
