...
% ANDERS / THOMAS
We here consider the situation where $\Lambda_1$, $\Lambda_2$, and $\Gamma$ are
nuisance parameters and an the estimator of interest is obtained from estimators
of these nuisance parameters. An example is an estimator of the cumulative
incidence curve, which can be obtained from estimators of $\Lambda_1$ and
$\Lambda_2$.
% THOMAS: Jeg ville så også tilføje: We give another example in Section 7.
...
In this section we report results from a simulation study without competing
risks. In the next section, we consider a setting with competing risks. Here we
first compare the state learner to an IPCW based discrete super learner that
uses either the Kaplan-Meier estimator or a Cox model to estimate the censoring
probability. We next compare the state learner to the super learner proposed by
\cite{westling2021inference}.
% ANDERS: kan du skrive her hvad forskellen er mellem og formaalet er
% med de to simulationsstudierscenarier?
% i den ene sammenligner vi med IPCW i den anden med westling, ik?
% Maaske en figur (flowchart) eller tabel kunne hjaelpe laeseren at faa overblik.
% Maaske separation i to subsections kunne hjaelpe yderliger
%
% THOMAS: Jeg ved ikke, om jeg ville sige at der er to simulationstudier, der er
% bare to ting vi undersøge med det samme studie. Data er genereret på samme
% måde til at undersøge de to spørgsmål. Har opdateret introen til afsnittet, er
% det bedre?

We generate data according to a distribution motivated from a real
dataset in which censoring depends on the baseline covariates. We
...
% ANDERS: det er uklar for mig hvordan
% de super learners valger/kombinerer de learners der ingaar i library: er det
% Brier score? over time?
% 
% THOMAS: De er diskrete super learner baseret på Score function med summary
% measure 'ibs', så det er integrated brier score.
...
\begin{figure}
  ...
  \caption[]{For the risk prediction models provided by each of the super
    ...
    uncertainty.
    % ANDERS: er det korrekt at det er averages across 1000 data sets?
    %
    % THOMAS: Ja. ...?
  }
  \label{fig:ipcw-fail}
...
% ANDERS: it is a bit lazy to write "for most sample sizes" :) it is
% slightly annoying that the y-axis scale changes in the 4
% figures. would that be better with Brier instead of IPA?
%
% THOMAS: No, it is worse with Brier score I seem to recall. I have fixed to
% y-axis horizontally now, it works well for the outcome model but poorly for
% the censoring. Let me know what you like the best. 
...
\begin{figure}
  ...
  % ANDERS: would be good to know how how many trainings set individuals were
  % right censored before the evaluation time point of the IPA. maybe of
  % interest how you chose the evaluation time point.
  %
  % THOMAS: Good point, but I think that the time of evaluation did not matter
  % too much. Of course, if you pick a very early time point where nothing has
  % happened, there is almost no difference in performance.
  \label{fig:zelefski}
\end{figure}
...
% ANDERS: har brug for en gentle intro saetning som fortaeller hvad der kommer:
% in this section we re-analyse the ... Det skal ogsaa vaere klart i starten af
% sektion af der kommer noget om average treatment effect.
%
% THOMAS: Yes, klart. Fungerer det her:
In this section we re-analyze the prostate cancer data analyzed by
\cite{kattan2000pretreatment}, which we introduced in
Section~\ref{sec:numer-exper}. The data consists of 1,042 patients and includes
a competing event in the form of death without tumor recurrence. We use the
state learner to rank learners of the cause-specific cumulative hazard
functions. We then use the highest ranked combination of learners and the
estimation strategy outlined in Section~\ref{sec:targeted-learning} to estimate
the average treatment effect of hormone therapy on death and tumor recurrence.

We consider death without tumor recurrence and recurrence of tumor as competing
risks. We include the fives learner \texttt{KM}, \texttt{Cox strata CS},
...
requires that \( \Lambda_j \) can be written as an integral with respect to
Lebesgue measure nor assumes a
% ANDERS: "density of \Lambda_j" er uklar hvordan er denne density defineret?
%
% THOMAS: Better now?
(semi-)parametric form for $\Lambda_j$. In the remainder of this
section we point to some limitations of our proposal and discuss
...
% ANDERS: er du enig med min omformulering af den sidste saetning?
%
% THOMAS: Yes, super godt. 
For settings without competing risks, our numerical studies suggest
that measuring the performance of \( F \) also leads to good
performance for estimation of \( S \). However, further research on
this issue, both numerical and theoretical, is warranted.
% ANDERS: ved ikke helt hvad du taenker man kan goer i fremtiden men synes
% egentig at den sidste saetning kunne godt slettes. :)
%
% THOMAS: Forstår ikke helt din kommentar. Hvilken sætning, den ovenover din
% kommentar eller den nedenunder? :)
...
\( F \) directly.
% ANDERS: how would one estimate F? using HAL? or RF? but such a learner would
% not provide risk prediction models or would it? if yes you could say it
%
% THOMAS: I don't think that estimators for F exists -- that is what Vi is
% working on, right? Given F we can always obtain \Lambda_1 and \Lambda_2
% (equation 8 or eq:7 in tex) and then we can get predictions from these.
