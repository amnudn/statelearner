...
training data set, and hence (the logarithm of zero is infitity) the
partial likelihood loss in the test set would be infitity.
...
\begin{proposition}
  ...
  \begin{equation*}
    F^* = \argmin_{F \in \mathcal{H}_{\mathcal{P}}} P_0{[\bar{B}_\tau(F, \blank)]}.
...
Theorem 2.3 in \cite{vaart2006oracle}.  We assume that we split the
...
% ANDERS / THOMAS
We consider 
anestimator which is obtained from
...
In this section we report results from two settings of our simulation
studies, both settings are without competing risks. In the next
section, we consider a setting with competing risks.
...
% de super learners valger/kombinerer de learners der ingaar i library: er det Brier score? over time?
...
\begin{figure}
  ...
  % ANDERS: would be good to know how how many trainings set individuals were right censored before
  % the evaluation time point of the IPA. maybe of interest how you chose the evaluation time point.
  \label{fig:zelefski}
\end{figure}
...
% ANDERS: har brug for en gentle intro saetning som fortaeller hvad der kommer: in this section we re-analyse the ...
%         Det skal ogsaa vaere klart i starten af sektion af der kommer noget om average treatment effect.
The prostate cancer data analyzed by \cite{kattan2000pretreatment},
which we introduced in Section~\ref{sec:numer-exper}, include a
competing event in the form of death without tumor recurrence. For
illustration, we fit the state learner to the original data set
consisting of 1,042 patients. We consider death without tumor
recurrence and recurrence of tumor as competing risks. We include the
fives learner \texttt{KM}, \texttt{Cox strata CS}, \texttt{Lasso},
\texttt{Elastic}, and \texttt{RF} which are described in
Table~\ref{tab:zel-library}. We use the same library of learners to
learn \( \Lambda_1 \), \( \Lambda_2 \), and $\Gamma$. In this case,
\( \Lambda_1 \) denotes the cause-specific cumulative hazard function
of tumor recurrence, and \( \Lambda_2 \) denotes the cause-specific
...
requires evaluation of the density of \( \Lambda_j \) nor assumes a
...
% ANDERS: ved ikke helt hvad du taenker man kan goer i fremtiden men synes egentig at den sidste saetning kunne godt slettes. :)
...
\( F \) directly.
% ANDERS: how would one estimate F? using HAL? or RF? but such a learner would not provide risk prediction models or would it? if yes you could say it 
