...
between intuitive jugdment and the law of chance
...
event time
distribution. % ANDERS: mangler afrundning efter sidste saetning. eller maaske kan saetningen flyttes laenger ned?
...
quantities. % ANDERS: hvad mener vi med observable quantities? distribution of the observed random variables?
...
\citep{van2011targeted}. % ANDERS: er der bedre referencer fra Mark paper/bog for discrete super learner?
In the presence of competing risks, our algorithm uses separate
libraries to model the hazard rates of the competing risks and the
...
continuous for all \( x \) and \( j \). %ANDERS: need a note here which says that this is not a limitation?
We denote by \(S\) the conditional
...
function for censoring. % ANDERS: can we not allow Gamma to be discrete?
...
log-likelihood loss.
% ANDERS: would be good to have a formula for a the negative partial log-likelihood loss showing that the result is infitity
...
models. % ANDERS: can the library really include multiple proportional hazards models? only one of them can be correct ...
% also, would be good to merge this paragraph with the previous somehow
...
tuple of learners of \( (\Lambda_1, \Lambda_2, \Gamma) \) based on how
well they jointly model the observed data. %ANDERS: would be good to add one sentence about how to obtain a prediction model from such a ranking.
...
\end{figure}
% ANDERS: Figuren er lavet med prodlim, ik? du kan angive nummer for boksene (0 = initial, 1 = cause 1, 2= cause 2 og jeg ville foretraekke 3 i stedet for -1 for censored)
...
% ANDERS: the next sentence requires conditional independent censoring and restriction to [0,\tau], right?
Because each quadruple
$(\Lambda_{1}, \Lambda_{2}, \Gamma, H)$ characterizes a distribution
\(P\in\mathcal P\) which in turn determines \( (F, H) \),
% ANDERS: and we should refer to equations (2) and (6) 
a learner for \( F \) can be constructed from learners of \( \Lambda_1 \),
...
Proposition~\ref{prop:stric-prop} can be derived from the fact that
the integrated Brier score (also called the continuous ranked
probability score) is a strictly proper scoring rule
\citep{gneiting2007strictly}. This implies that if we minimise the
average loss of the integrated Brier score, we recover the parameters
of the data-generating distribution. Specifically, this implies that
the oracle of a state learner is consistent if the library of learners
contains at least one learner that is consistent for estimation of
\( F \). Recall that the function \(F\) implicitly depends on the data
generating probability measure \(P\in\mathcal P\) but that this was
suppressed in the notation. We now make this dependence explicit by
writing \(F_0\) for the function which is obtained by substituting a
specific \(P_0\in\mathcal{P}\) for \(P\) in equation
\eqref{eq:transition}.
...
\begin{proposition}
  ...
  \end{equation*}
  %ANDERS: would be good to define the set over which F is minimized with a symbol rather than only words
...
We now establish a finite sample oracle result for the state learner.
Our Corollary~\ref{cor:oracle-prop} is in essence a special case of Theorem 2.3
in \cite{vaart2006oracle}. % ANDERS: is Theo 2.3 correct?
We assume
...
% ANDERS: would be good to separate 2-3 subsection in this section. here is a vague proposal. above could be under \subsection{Oracle property of the state learner}
...
% ANDERS: at this point we have not defined "targeted estimator" does fisher 2021 define this? Bickel does not ...
is that it
is asymptotically equivalent to a sum of i.i.d.\ random variables plus a second
order remainder term \citep{bickel1993efficient,fisher2021visually}. For the
setting with competing risks, the remainder term is dominated by terms
of the form
\begin{equation}
  \label{eq:dr-term}
  P{\left[
...
conditional independent censoring and positivity, $\theta$ is
identifiable from \( \mathcal{P} \) which means that there exists an
...
\end{equation*}
Under a set of
% ANDERS: better to give these assumptions a name: positivity, ... and refer to Helene's paper?
