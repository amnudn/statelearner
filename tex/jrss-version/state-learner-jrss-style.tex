...
In this section we report results from a simulation study. In the
first part we consider estimation of the conditional survival
function. Here we compare the state learner to two IPCW based discrete
super learners that use either the Kaplan-Meier estimator or a Cox
model to estimate the censoring probability
\cite{gonzalez2021stacked}.  In the second part we compare the state
learner to the super learner proposed by \cite{westling2021inference}.
...
% ANDERS: describe that you use a single evaluation time point and say how many censored, event happen before it.
