...
sim_fun <- function(seed){
    ...
    raw_calc = do.call(rbind, lapply(ps, function(pp){
        ...
        do.call(rbind, lapply(ns, function(nn){
            ...
            ## Unnice hack...
            if(pp == 0){
                ipcw_lasso = GLMnet(Surv(time, status)~X1, data = train0)
                ipcw_elastic = GLMnet(Surv(time, status)~X1, data = train0, alpha = .5)
            }
            if(pp == 1){
                ipcw_lasso = GLMnet(Surv(time, status)~X1+X2, data = train0)
                ipcw_elastic = GLMnet(Surv(time, status)~X1+X2, data = train0, alpha = .5)
            }
            if(pp == 5){
                ipcw_lasso = GLMnet(Surv(time, status)~X1+X2+X3+X4+X5+X6, data = train0)
                ipcw_elastic = GLMnet(Surv(time, status)~X1+X2+X3+X4+X5+X6, data = train0, alpha = .5)
            }
            if(pp == 10){
            ...
            out0 = eval_sl(train0, true_test, eval_time = eval_times,
                           stateL_learners = list(
                               ...
                               elastic = list(model = "GLMnet", x_form = x_form0, alpha = 0.5),
