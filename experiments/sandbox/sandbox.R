...
tar_load(zel_sim2_1, store = here("experiments/_targets"))
...
ggplot(summ_zel_sim2_1[time == 36 & type == "event" & SL != "survSL"],
    ...
    theme_bw() +
    geom_errorbar(position=position_dodge(width = dd_ww),aes(ymin = IPA-1.96*se, ymax = IPA+1.96*se), width = .05, alpha = .5) + 
    geom_line(position=position_dodge(width = dd_ww)) + geom_point(position=position_dodge(width = dd_ww)) +
    scale_x_continuous(trans='log2') +
    facet_wrap(~sim_set, ncol = 2, scales = "free_y")

ggplot(summ_zel_sim2_1[time == 36 & SL %in% c("survSL", "statelearner", "oracle")],
       aes(x = n_obs, y = IPA, col = SL)) +
    theme_bw() +
    geom_errorbar(position=position_dodge(width = dd_ww),aes(ymin = IPA-1.96*se, ymax = IPA+1.96*se), width = .05, alpha = .5) + 
    geom_line(position=position_dodge(width = dd_ww)) + geom_point(position=position_dodge(width = dd_ww)) +
    scale_x_continuous(trans='log2') +
    facet_wrap(type~sim_set, ncol = 2, scales = "free_y")
