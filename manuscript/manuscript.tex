...
review them here and contrast our suggestion. Machine learning based on
right-censored data commonly uses the partial log-likelihood as a loss function
...
splitting (cross-validation) in continuous time because this loss assigns an
infinite value when applied to a test set time point which does not occur in the
learning set as soon as the learner predicts piece-wise constant cumulative
hazard functions. This is the case for prominent survival learners including the
Kaplan-Meier estimator, the random survival forest, and the semi-parametric Cox
regression model, which hence cannot be included in the library of the super
learner proposed by \cite{polley2011-sl-cens}.

When a proportional hazards model is assumed, the baseline hazard function can
be profiled out of the likelihood \citep{cox1972regression}. The cross-validated
partial log-likelihood loss \citep{verweij1993cross} has therefore been
suggested as a loss function for super learning \citep{golmakani2020super}. This
choice of loss function restricts the library of learners to include only Cox
proportional hazards models, and hence excludes many learners such as, e.g.,
random survival models, additive hazards models and accelerated failure time
models.
% A lesser
% known fact is that a super learner constructed with the partial
% log-likelihood loss implicitly depends on the censoring distribution
% \citep{hjort1992inference,whitney2019comment}.

Other attempts at super learning with right-censored data use an inverse
probability of censoring weighted (IPCW) loss function
...
% An IPCW loss function is attractive because the associated risk does
% not depend on the censoring distribution but describes a feature of
% the population of interested. Similar results can be obtained using
...
% When the data-generating mechanism is complex and not
% well-understood, pre-specification of the censoring distribution is a
% challenge.
% Inverse probability of censoring weighted loss has been studied for
% decades \citep{graf1999assessment, hothorn2006survival,
  % gerds2006consistent}. A disadvantage of inverse probability of
% censoring weighted loss estimators is that they require a
% pre-specified model for the censoring distribution.
% Recently \cite{westling2021inference} proposed a procedure which
% iterates between super learning of the censoring distribution and
% super learning of the event time distribution.

A recent approach which avoids a pre-specified censoring model was proposed
independently by \cite{han2021inverse} and \cite{westling2021inference}. In both
articles, the authors suggest to iterate between learning of the outcome model
and learning of the censoring model using IPCW loss functions. However, no
general theoretical guarantees seem to exist for this procedure. For instance,
the procedure is not guaranteed to converge to the true data-generating
mechanism \citep{munch2024thesis}.

Contrary to the super learners considered by \cite{polley2011-sl-cens} and
\cite{golmakani2020super}, the state learner poses no restrictions on the type
of learners that can be included in the library. Contrary to IPCW, censoring
unbiased transformations, or pseudo-value based super learners, the state
learner does not require a pre-specified censoring distribution, but constructs
a super learner for both the outcome and the censoring distribution. Contrary to
the proposals by \cite{han2021inverse} and \cite{westling2021inference}, the
state learner is backed by theoretical guarantees.
...
% ANDERS: hedder det ikke Nelson-Aalen i stedet for Kaplan-Meier?
%         kan du skrive klart hvad outcome er i denne foerste del? er det combined endpoint som man kalder for tumor-free survival?  
...
% ANDERS: er du sikker paa at det er ALL levels of interaction, ogsaa higher order?
%         der mangler en reference for lasso model
...
% ANDERS: er du enig i min omformulering af forgaaende saetning?
...
for estimation of \( S \).
% ANDERS: den forgaaende paragraf gentager det samme 2-3 gang. kan du forkorte en gang?
